<template>
  <v-app dark>
    <v-app-bar fixed app>
      ...
      <b>Teemops</b>
      <v-spacer />
      <account-menu v-if="status != null" :status="status" v-on:logout="logoutAction"
        v-on:login="loginAction"></account-menu>
    </v-app-bar>
    <v-main>
      <v-container>
...
export default Vue.extend({
   ...
   */
  async mounted() {
    console.log('This is simple layout mounted')
    try {
      //check if user has loggedin and been here (e.g. in this browser before)
      console.log('Been here', this.beenHere)
      if (this.beenHere) {
      ...
      console.log('Error in simple layout mounted', e)
      this.status = false
    }
  },
  methods: {
