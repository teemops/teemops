<template>
  <div>
    <v-row>
      <v-col cols="6">
        <v-text-field label="Enter email address" :rules="emailValidation" v-model="email"></v-text-field>

        <v-text-field v-if="showPass" type="password" label="Enter password" :rules="passValidation"
          v-model="password"></v-text-field>
        <v-btn v-if="showPass" color="primary" v-on:click="loginMe" :disabled="email.length == 0 && password.length == 0">
          Register
        </v-btn>
      </v-col>
    </v-row>
