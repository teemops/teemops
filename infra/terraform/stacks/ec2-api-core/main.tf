terraform {
  ...
  backend "s3" {
    ...
    key     = "stacks/ec2-api-core/tfstate"
    region  = "us-east-1"
    encrypt = true
  }
}
...
module "asg" {
  ...
  iam_profile      = module.ec2_iam_role.my_profile_arn
  iam_profile_name = module.ec2_iam_role.my_profile_name
  #key_pair        = "${var.env}-${var.region_name}-key"
